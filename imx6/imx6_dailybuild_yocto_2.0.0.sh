...
BSP_URL="https://github.com/timliangtw/adv-arm-yocto-bsp.git"
BSP_BRANCH="imx-4.1-krogoth"

U_BOOT_VERSION="2016.03"
U_BOOT_URL="https://github.com/timliangtw/uboot-imx6.git"
U_BOOT_BRANCH="imx_v2016.03_4.1.15_2.0.0_ga"
U_BOOT_PATH="sources/meta-advantech/meta-fsl-imx6/recipes-bsp/u-boot/u-boot-imx_$U_BOOT_VERSION.bbappend"

KERNEL_VERSION="4.1.15"
KERNEL_URL="https://github.com/timliangtw/linux-imx.git"
KERNEL_BRANCH="imx_4.1.15_2.0.0_ga"
KERNEL_PATH="sources/meta-advantech/meta-fsl-imx6/recipes-kernel/linux/linux-imx_$KERNEL_VERSION.bbappend"

META_ADVANTECH_URL="https://github.com/timliangtw/meta-advantech.git"
META_ADVANTECH_PATH="sources/meta-advantech"
META_ADVANTECH_BRANCH="krogoth"

YOCTO_BSP_BRANCH="imx-4.1-krogoth"

DEFAULT_DEVICE="imx6qrsb4410a1"
BUILDALL_DIR="build_all"

FSL_IMAGE_NAME="fsl-image-qt5"

MFGTOOLS_URL="https://github.com/timliangtw/Mfgtools.git"
MFGTOOLS_BRANCH="4.1.15"
...
function check_tag_and_checkout()
{
        ...
        if [ -d "$ROOT_DIR/$FILE_PATH" ];then
                ...
                META_TAG=`git tag | grep $VER_NUM`
                if [ "$META_TAG" != "" ]; then
                        echo "[ADV] meta-advantech has been tagged ,and check to this $VER_NUM version"
                        git checkout $VER_NUM
                        git tag --delete $VER_NUM
                        git push --delete $REMOTE_URL refs/tags/$VER_NUM
                else
...
function check_tag_and_replace()
{
        ...
        FILE_PATH=$2
        REMOTE_BRANCH=$3
        HASH_ID=`git ls-remote $REMOTE_URL $VER_NUM | awk '{print $1}'`
        if [ "$HASH_ID" != "" ]; then
...
function auto_add_tag()
{
		...
		if [ -d "$FILE_PATH/$DIR/git" ];then
			echo "[ADV] Add tag $VER_NUM on $REMOTE_URL"
			cd $FILE_PATH/$DIR/git
			git tag -a $VER_NUM -m "official build"
			git push $REMOTE_URL $VER_NUM
			cd $CURR_PATH
		else
...
function create_branch_and_commit()
{
        ...
        if [ -d "$ROOT_DIR/$FILE_PATH" ];then
                cd $ROOT_DIR/$FILE_PATH
                git checkout -b $VER_NUM
                git add .
                git commit -m "official build $VER_NUM"
                git push $REMOTE_URL $VER_NUM
                cd $CURR_PATH
        else
                echo "[ADV] Directory $ROOT_DIR/$FILE_PATH doesn't exist"
...
function create_xml_and_commit()
{
        if [ -d "$ROOT_DIR/.repo/manifests" ];then
                ...
                git checkout $YOCTO_BSP_BRANCH
                cp default.xml $VER_NUM.xml
                sed -i "s/\"meta-advantech\" revision=\"$META_ADVANTECH_BRANCH\"/\"meta-advantech\" revision=\"$VER_NUM\"/g" $VER_NUM.xml
                git add $VER_NUM.xml
                git commit -m "official build $VER_NUM"
                git push
                cd $CURR_PATH
...
function generate_table()
{
        ...
        echo "U-Boot,U-Boot ${U_BOOT_VERSION}-${VERSION}" >> $FILENAME.csv
        echo "Build Number,${VERSION}" >> $FILENAME.csv
...
function add_version()
{
        ...
        echo "UBOOT_LOCALVERSION = \"-$VERSION\"" >> $ROOT_DIR/sources/meta-advantech/meta-fsl-imx6/recipes-bsp/u-boot/u-boot-imx_${U_BOOT_VERSION}.bbappend
        ...
        echo "LOCALVERSION = \"-$VERSION\"" >> $ROOT_DIR/sources/meta-advantech/meta-fsl-imx6/recipes-kernel/linux/linux-imx_${KERNEL_VERSION}.bbappend
}
...
function building()
{
        ...
        if [ "$1" == "populate_sdk" ]; then
                if [ "$FSL_IMAGE_NAME" == "fsl-image-qt5" ]; then
                        echo "[ADV] bitbake meta-toolchain-qt5"
                        bitbake meta-toolchain-qt5
                else
                        echo "[ADV] bitbake $FSL_IMAGE_NAME -c populate_sdk"
                        bitbake $FSL_IMAGE_NAME -c populate_sdk
                fi
        elif [ "x" != "x$2" ]; then
                bitbake $1 -c $2 -f
        else
...
function rebuild_u-boot()
{
        ...
        building u-boot-imx cleansstate
        building u-boot-imx
        bitbake $FSL_IMAGE_NAME -c rootfs -f
        cd  $CURR_PATH
}
...
# Generate mksd-linux.sh
cat << \DB_EOF > mksd-linux.sh
#!/bin/bash
...
# Generate mkspi-advboot.sh
cat << EOF > mkspi-advboot.sh
#!/bin/bash
...
function prepare_images()
{
        ...
        case $IMAGE_TYPE in
                "sdk")
                        ...
                        ;;
                "normal")
                        FILE_NAME=${FSL_IMAGE_NAME}"-"${KERNEL_CPU_TYPE}${PRODUCT}"*.rootfs.sdcard"
                        ...
                        ;;
                "eng")
                        FILE_NAME=`readlink $DEPLOY_IMAGE_PATH/"${FSL_IMAGE_NAME}-${KERNEL_CPU_TYPE}${PRODUCT}.sdcard" | cut -d '.' -f 1`".rootfs.eng.sdcard"
                        cp $DEPLOY_IMAGE_PATH/$FILE_NAME $OUTPUT_DIR
                        if [ -e $OUTPUT_DIR/$FILE_NAME ]; then
...
ROOT_DIR="$VER_NUM"_"$DATE_PATH"
mkdir $ROOT_DIR
echo "ROOT_DIR=$ROOT_DIR"

case $BACKEND_TYPE in
    "wayland")
        FSL_IMAGE_NAME="fsl-image-weston"
        ;;
    "fb")
        FSL_IMAGE_NAME="fsl-image-gui"
        ;;
    *)
...
echo "FSL_IMAGE_NAME=$FSL_IMAGE_NAME"
